#import "RemappingSignatureAdapter.h"

@implementation RemappingSignatureAdapter

+ (RemappingSignatureAdapter*)valueWithSignatureVisitor:(id)arg0 withRemapper:(id)arg1 {
	return nil;
}

- (void)visitClassTypeWithString:(NSString*)arg0 {

}

- (void)visitInnerClassTypeWithString:(NSString*)arg0 {

}

- (void)visitFormalTypeParameterWithString:(NSString*)arg0 {

}

- (void)visitTypeVariableWithString:(NSString*)arg0 {

}

- (id)visitArrayType {
	return nil;
}

- (void)visitBaseTypeWithChar:(char)arg0 {

}

- (id)visitClassBound {
	return nil;
}

- (id)visitExceptionType {
	return nil;
}

- (id)visitInterface {
	return nil;
}

- (id)visitInterfaceBound {
	return nil;
}

- (id)visitParameterType {
	return nil;
}

- (id)visitReturnType {
	return nil;
}

- (id)visitSuperclass {
	return nil;
}

- (void)visitTypeArgument {

}

- (id)visitTypeArgumentWithChar:(char)arg0 {
	return nil;
}

- (void)visitEnd {

}

@end
